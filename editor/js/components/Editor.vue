...
export default {
    ...
    methods: {
         ...
         */
        initPropMap: function(layers) {layers
        ...
        popup: function(title, message, actions) {
            Vue.set(this, 'modal', {
                ...
                actions: actions.map((elem) => Object.assign({
                    name: elem.name,
                    action: () => {
                        Vue.delete(this.modal, 'show'); (elem.action || (() => undefined))();
                    },
                })),
        ...
        prompt: function(title, initialValue, actions) {
            Vue.set(this, 'modal', {
                ...
                actions: actions.map((elem) => Object.assign({
                    name: elem.name,
                    action: (value) => {
                        Vue.delete(this.modal, 'show'); (elem.action || ((value) => undefined))(value);
                    },
                })),
                show: true, // show dialog
            });
        ...
        operateZooming: function(operation) {
            this.zoomInfo[
                {'Editor': 'networkGraph', 'Learning Curve': 'learningCurve', 'Trade-off Graph': 'tradeOffGraph'}[operation.name]
...
.app-row, .app-col { overflow: hidden; position: absolute; }
.app-row { left: 0; right: 0; }
.app-col { top: 0; bottom: 0; }
