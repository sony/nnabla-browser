...
const svgAreaOperatorCtor = function () {
  ...
  const adjustSvgSize = () => {
    ...
    const layersDOM = svgLayers.node().getClientRects()[0]
  ...
  const getOverlapLayerPosition = ({ x: x1, y: y1 }, layerIndex) => {
    ...
    for (let i = 0; i < numLayers; i++) {
      ...
      const position = getLayerPosition(i)
  ...
  const layerDragging = function (v, i) {
    ...
    const x = currentX + d3.event.dx; const y = currentY + d3.event.dy
    d3.select(this)
      .attr('transform', `translate(${x}, ${y})`)

    // redraw all links
    for (const link of self.connectedLinks) {
      link.update({ x, y })
  ...
  const layerDragEnd = function (v, layerIndex) {
    ...
    while (true) {
      ...
      const overlapLayerPosition = getOverlapLayerPosition({ x, y }, layerIndex)
