...
export default Vue.extend<DataType, {}, ComputedType, {}>({
    ...
    zoom: {
      inserted: function (el, binding, vNode: VNode) {
        const getTransformInfo = (el: HTMLElement) => {
          ...
          if (!graph) return { graph: null, translateX: 0, translateY: 0, scale: 1 }
  ...
  methods: {
    ...
    getAssistDotsStyle: () => {
      ...
      const moveToRight = Math.max(Math.ceil((scroller as Element).scrollLeft / grid), 0) * grid
      const moveToBottom = Math.max(Math.ceil((scroller as Element).scrollTop / grid), 0) * grid
